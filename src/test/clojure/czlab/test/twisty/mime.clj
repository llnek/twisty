...
(deftest czlabtesttwisty-mime
  ...
  (testing
    "related to: digest"
    (is (let [f (t/digest<> (c/bytesit "heeloo world") :sha-1)]
          (and f (> (.length f) 0))))

    (is (let [f (t/digest<> (c/bytesit "heeloo world") :md5)]
          (and f (> (.length f) 0))))

    (is (let [f (t/digest<> (c/bytesit "heeloo world") :sha-1)
              g (t/digest<> (c/bytesit "heeloo world") :md5)]
          (if (= f g) false true)))

    (is (let [f (t/digest<> (c/bytesit "heeloo world") :sha-1)
              g (t/digest<> (c/bytesit "heeloo world") :sha-1)]
          (= f g)))
